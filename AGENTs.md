...
- Demand generation:
  ...
  - Sample origins and destinations from fixed hotspot-weight maps blended with uniform demand: low `hs` is nearly uniform, high `hs` concentrates origins near `(2,2)` and destinations near `(7,7)`.
...
- Mode 1: fixed route
  <!-- - dispatch all vehicles from the depot `(1,5)` every `15` intervals, starting at time `0`, followig the same loop. -->
...
- Mode 4: hub-and-spoke
  - Use four fixed spokes from hub to the north, east, south, and west 4 spokes.
  - Vehicles shuttle hub-to-spoke-to-hub and are dispatched from the hub in deterministic round-robin spoke order.
